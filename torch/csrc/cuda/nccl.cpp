...
void all2all_single_equal_split(
  ...
  const auto* sendbuff = reinterpret_cast<const char*>(input.data_ptr());
...
void gather(
  ...
  const auto* sendbuff = reinterpret_cast<const char*>(inputs.data_ptr());
  ...
  if (cur_rank == root) {
    for (const auto r : c10::irange(numranks)) {
      if (r != root) {
        ...
        const auto* sendbuff = reinterpret_cast<const char*>(inputs[r].data_ptr());
