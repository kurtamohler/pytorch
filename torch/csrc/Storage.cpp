...
void refcounted_deleter(void* ctx_) {
  RefcountedDeleterContext& ctx = *reinterpret_cast<RefcountedDeleterContext*>(ctx_);
  ctx.refcount--;
  if (ctx.refcount == 0) {
...
void maybeReplaceDataPtrWithShared(c10::Storage storage) {
  c10::DataPtr& data_ptr = storage.data_ptr();
  ...
  RefcountedDeleterContext* refcount_ctx = new RefcountedDeleterContext(other_ctx, other_deleter);
...
PyObject* THPStorage_Wrap(c10::Storage storage) {
  ...
  if (c10::impl::HermeticPyObjectTLS::get_state()) {
    ...
    bool has_pyobj = pyobj_slot->check_pyobj(pyobj_slot->pyobj_interpreter()).has_value();
    bool is_same_interpreter = pyobj_slot->pyobj_interpreter() == getPyInterpreter();
