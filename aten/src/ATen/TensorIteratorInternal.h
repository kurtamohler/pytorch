...
inline void serial_for_each(
    ...
    size_t nconst,
    typename TensorIteratorBase::new_loop2d_t loop,
    Range range) {
  const auto ndim = shape.size();
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
      mutable_strides.size() == nmutable * std::max(size_t{2}, ndim));
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
      const_strides.size() == nconst * std::max(size_t{2}, ndim));
  ...
  if (ndim <= 1) {
    if (range.begin == 0) {
      ...
      c10::SmallBuffer<char*, 4> mutable_ptrs(nmutable);
      c10::SmallBuffer<const char*, 4> const_ptrs(nconst);
      get_data_ptrs(
          mutable_ptrs.data(),
          {mutable_base_ptrs, nmutable},
          mutable_strides,
          {range.begin});
      get_data_ptrs(
          const_ptrs.data(),
          {const_base_ptrs, nconst},
          const_strides,
          {range.begin});
      loop(
          mutable_ptrs.data(),
