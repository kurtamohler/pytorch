...
TEST(TensorIteratorTest, ForEachMutableInput) {
  ...
  EXPECT_TRUE(out.eq(b).all().item<bool>());
  EXPECT_TRUE(a.eq(b + 15).all().item<bool>());
}

// TODO: Write a test with TensorIterator::split. I think
// that's where the segfault happens.
