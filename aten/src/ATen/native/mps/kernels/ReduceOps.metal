...
#define MAX_THREADGROUP_SIZE 256
...
struct norm_cast_functor {
  template <typename T_from, typename T_to, enable_if_t<is_complex_v<T_from> && !is_complex_v<T_to>, bool> = true>
...
static uint32_t get_input_offset(uint32_t reduction_element_idx, uint32_t reduction_idx, constant NormParams<>& params) {
  ...
  for (int32_t dim = params.ndim - 1; dim >= 0; dim--) {
    ...
    // Otherwise, this dim is being reduced, so we index by `reduction_element_idx`
    } else {
...
kernel void norm(
  ...
  for (uint32_t reduction_element_idx = tid; reduction_element_idx < params.reduction_size; reduction_element_idx += tptg) {
    TC input_elem = norm_cast_functor().call<TI, TC>(input[get_input_offset(reduction_element_idx, tgid, params)]);
  ...
  for (uint32_t stride = 1; stride < tptg; stride *= 2) {
    if ((tid % (stride * 2) == 0) && (tid + stride) < tptg) {
      if (params.p == INFINITY) {
        shared_output[tid] = max(shared_output[tid], shared_output[tid + stride]);
      } else if (params.p == -INFINITY) {
        shared_output[tid] = min(shared_output[tid], shared_output[tid + stride]);
      } else {
  ...
  if (tid == 0) {
    ...
    if (params.p != 0 && params.p != 1 && params.p != INFINITY && params.p != -INFINITY) {
      output_elem = static_cast<TA>(::precise::pow(output_elem, static_cast<TA>(1 / params.p)));
    }
