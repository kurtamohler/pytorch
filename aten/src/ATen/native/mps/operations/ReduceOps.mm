...
static void impl_func_norm_mps_new(const Tensor& input,
  ...
  std::vector<int64_t> output_view_shape = dim.empty() ?
    std::vector<int64_t>(input_view.dim(), 1) :
  ...
  dispatch_sync_with_rethrow(stream->queue(), ^() {
    @autoreleasepool {
      ...
      auto pipeline_state = lib.getPipelineStateForFunc(fmt::format("norm_{}_{}_{}", scalarToMetalTypeString(input_view), scalarToMetalTypeString(input_cast_dtype), scalarToMetalTypeString(output_view)));
      ...
      constexpr uint32_t MAX_THREADGROUP_SIZE = 256;
...
(const Tensor& self, const OptionalScalarRef opt_p, IntArrayRef dim, bool keepdim, const Tensor& result) {
  impl_func_norm_mps_new(self, opt_p, dim, keepdim, /*dtype=*/std::nullopt, result);
}
...
(const Tensor& self,
 ...
 ScalarType dtype,
 const Tensor& result) {
  impl_func_norm_mps_new(self, opt_p, dim, keepdim, dtype, result);
}
...
(const Tensor& self,
 ...
 std::optional<ScalarType> opt_dtype,
 const Tensor& result) {
  impl_func_norm_mps_new(self, scalar_ord, opt_dim.value_or(IntArrayRef{}), keepdim, opt_dtype, result);
}
...
Tensor _cdist_forward_mps(const Tensor& x1, const Tensor& x2, const double p, std::optional<int64_t> compute_mode) {
  ...
  impl_func_norm_mps_new(diff, OptionalScalarRef(p), makeArrayRef<int64_t>(-1), /*keepdim=*/false, /*dtype=*/std::nullopt, result);
  return result;
}
